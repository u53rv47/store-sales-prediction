...
class DataValidation:
    ...
    def initiate_data_validation(self) -> artifact_entity.DataValidationArtifact:
        try:
            ...
            logging.info(f"drop null value colums in  train dataframe")
            train_df = self.drop_missing_values_columns(
                df=train_df, report_key_name="missing_values_within_train_dataset")
            logging.info(f"drop null value colums in  test dataframe")
            ...
            if train_df_columns_status:
                logging.info(
                    f"As all columns are avialable in train df hence we are going to detect data drift")
                self.data_drift(base_df=base_df, current_df=train_df,
                                report_key_name="data_drift_within_train_dataset")
            if test_df_columns_status:
                logging.info(
                    f"As all columns are avialable in test df hence we are going to detect data drift")
